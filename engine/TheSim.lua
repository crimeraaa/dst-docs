...
-- Closes the game. That's it. It does run garbage collection and stuff.
function TheSim:Quit() 
    print("Quitting game...") 
end

-- The actual Lua `print` function resides here. Implicitly calls `tostring` for all arguments.
-- 
-- Writes to log files (client or server) but not to client's ingame console.
-- If on a dedicated server, it also writes to the terminal.
---@param ... any
...
function TheSim:ShouldInitDebugger() 
    return false
end
...
---@return table
function TheSim:FindEntities (x, y, z, radius, must_tags, cant_tags, oneof_tags)
    return {} 
end
...
---@param skin_id? any
---@param creator? any
function TheSim:SpawnPrefab(name, skin, skin_id, creator)
    return 123456
end

-- Seems to load prefab definitions into memory by modifying the global 
-- `Prefabs` table, which uses `string` keys.
--
-- See definition of `DebugSpawn`, (`scripts/util.lua:22`).
---@param tbl string[] 
function TheSim:LoadPrefabs(tbl)
    -- I'm certain the actual internals are more intricate than this,
    -- as it's possible for `Prefabs[v]` to be `nil`.
    for i, v in ipairs(tbl) do
        if not Prefabs[v] then
            Prefabs[v] = true
        end
    end
end
...
function TheSim:SetTimeScale(multiplier)
...
-- I am unsure what this number means.
function TheSim:GetTickTime() 
    return 0.033333335071802
end

-- Retrives the current time scale, typically as previously set by 
-- `TheSim:SetTimeScale`. See `engine/TheSim.lua`.
function TheSim:GetTimeScale()
    return 1 
end

-- Seems to get the time since the current session was started.
-- Increments by 1000 every second. Perhaps nanoseconds?
function TheSim:GetRealTime() 
    return 237000
end
